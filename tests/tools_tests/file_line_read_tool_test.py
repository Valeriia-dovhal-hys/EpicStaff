...
class TestFileLineReadTool:
    ...
    def test_line_read_tool_with_crewai(
        ...
        task = Task(
            description=f"""Read all lines starting with {line_number} from {filepath.as_posix()}""",
            agent=agent,
            expected_output=f"""The response in the 
            following format using relative path:
            "I read from the file {filename} in {filepath.as_posix()} and here are the lines I found:\n{expected}" (without "")
        ...
        assert (
            output == f"I read from the file {filename} in {filepath.as_posix()} and here are the lines I found:\n{expected}"
        )
