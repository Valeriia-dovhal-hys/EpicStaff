...
class TestFileAppendTool:
    ...
    def test_append_tool_with_crewai(self, agent, append_file_tool):
        ...
        task = Task(
            description=f"""Append the text {text_to_append} to {file_path.as_posix()}""",
            agent=agent,
            expected_output=f"""The response in the 
            following format using relative path:
            "I appended a text {text_to_append} to {file_path.as_posix()}." (without "")
        ...
        assert output == f"I appended a text {text_to_append} to {file_path.as_posix()}."
