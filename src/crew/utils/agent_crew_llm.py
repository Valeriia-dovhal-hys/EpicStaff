...
from src.utils.ollama_loader import OllamaLoader
...
from src.utils.groq import TokenThrottledChatGroq
...
def get_llm(
    ...
    **kwargs,
    ...
    if provider.lower() == "anthropic":
        logger.info(
            f"Using Anthropic model '{model_name}' with temperature {temperature}."
        )
        try:
            ...
            print(
                f"Hey, I've failed to configure Anthropic model '{model_name}'. Could you check if the API KEY is set? :\n{e}"
            )
            return None
    ...
    if provider.lower() == "azure_openai":
        logger.info(
            f"Trying {provider} model '{model_name}' with temperature {temperature},"
        ...
        try:
            ...
            print(
                f"Hey, I've failed to configure Azure OpenAI model '{model_name}'. Could you check if the API KEY is set? :\n{e}"
            )
            return None
    ...
    if provider.lower() == "openai":
        ...
        try:
            ...
            print(
                f"Hey, I've failed to configure OpenAI model '{model_name}'. Could you check if the API KEY is set? :\n{e}"
            )
            return None
    ...
    if provider.lower() == "openai_compatible":
        logger.info(
            f"Trying {provider} model '{model_name}' with temperature {temperature}, base_url {base_url}"
        )
        try:
            return ChatOpenAI(
                model=model_name,
    ...
    if provider.lower() == "groq":
        ...
        try:
            ...
            print(
                f"Hey, I've failed to configure Groq model '{model_name}'. Could you check if the API KEY is set?\n{e}"
            )
            return None
    ...
    if provider.lower() == "huggingface":
        logger.info(
            f"Trying {provider} repo_id '{model_name}' with hugingfacehub_api_token. via env"
        )
        try:
            ...
            print(
                f"Hey, I've failed to configure HuggingFace model '{model_name}'. Could you check if the API KEY is set? \n{e}"
            )
            return None
    # Ollama
    ...
    logger.error(
        f"Provider '{provider}' not recognized. Please use one of the supported providers: 'anthropic', 'azure_openai', 'openai', 'huggingface', 'ollama'."
    )
    return None
