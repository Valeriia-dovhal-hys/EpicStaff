...
class ImportToolDataBuilder:
    ...
    def process_callable(self, callable_data: Dict[str, Any]) -> Callable:
        ...
        if not module_path:
            module_path = None
            # raise ValueError(
            #     f"Module path for class '{callable_name}' not found in tools_paths.json."
            # )
        ...
        return Callable(
            module_path=module_path, # Optional
    ...
    def get_import_class_data(self, key: str) -> ImportToolData:
        ...
        if not module_path:
            raise ValueError(
                f"Module path for class '{class_name}' not found in tools_paths.json."
            )
