...
class ToolsMapping:
    def __init__(self, tools_df, models_df):
        self.tool_registry = (
            CallableRegistry()
        )  ## Registry of all callables from all modules defined in tools_config.py...
    ...
    def load_tools(self, tools_df, models_df):
        ...
        for idx, row in tools_df.iterrows():
            ...
            logger.info(
                f"Loading tool '{tool_name}' with base name '{base_name}' and arguments '{arguments.group(1) if arguments else ''}'."
            )
            ...
            if callable(class_or_func):
                ...
                logger.info(
                    f"Creating tool '{tool_name}' with allable {class_or_func} and arguments '{args}' and keyword arguments '{kwargs}'."
                )
                ...
                logger.error(
                    f"Callable for '{base_name}' is not a function or class constructor."
                )
