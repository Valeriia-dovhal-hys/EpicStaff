...
class FileToolSchema(FixedFileToolSchema):
    ...
    line_number: int = Field(
        None,
        description="The line number to start reading from. Line numbers are 1-based.",
    )
    num_lines: Optional[int] = Field(
        None, description="The number of lines to read starting from line_number."
    )
    expected_text: str = Field(
        None, description="The text expected to be on the specified line for editing."
    )
    new_text: str = Field(
        None,
        description="The new text to replace the existing text on the specified line.",
    )
...
class FileTool(BaseTool):
    ...
    description: str = (
        "Manage 'append', 'edit', 'create', 'count_lines', 'read', 'create' file operations"
    )
