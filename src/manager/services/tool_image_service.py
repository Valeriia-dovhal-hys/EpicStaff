...
class ToolImageService:
    ...
    def build_image(self, image_name: str) -> Image:
        ...
        logger.info(f"Image built successfully with name: {import_tool_data.image_name}")
        return image
    ...
    def get_or_build_tool_alias(self, tool_alias: str) -> Image:
        ...
        logger.info(f"No local image found for alias {tool_alias}, attempting to pull from DockerHub.")
        image = self.pull_from_dockerhub(image_name)
        if image:
            return image

        logger.info(f"Image for alias {tool_alias} not found on DockerHub. Building new image.")
