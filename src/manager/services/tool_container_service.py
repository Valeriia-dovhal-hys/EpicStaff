...
class ToolContainerService:
    ...
    def find_running_containers_by_image_name(self, image_name) -> list[Container]:
        containers = self.docker_client.containers.list(filters={"ancestor": image_name})
        logger.info(f"Found {len(containers)} running container(s) for image name: {image_name}")
        return containers
    ...
    def request_class_data(self, tool_alias: str) -> dict:
        ...
        if not container:
            logger.info(f"Container not found for tool alias {tool_alias}, starting a new container.")
    ...
    def request_run_tool(self, tool_alias: str, run_tool_params_model: RunToolParamsModel) -> dict:
        container = self.get_running_tool(tool_alias=tool_alias)
        if not container:
            logger.info(f"Container not found for tool alias {tool_alias}, starting a new container.")
            container = self.run_container_by_tool_alias(tool_alias=tool_alias)
        ...
        try:
            ...
            raise RuntimeError(f"HTTP error occurred while running tool {tool_alias}: {e}")
        except RequestException as e:
            logger.error(f"Request error occurred while running tool {tool_alias}: {e}")
            raise RuntimeError(f"Request error occurred while running tool {tool_alias}: {e}")
    ...
    def run_container(self, image: Image, container_name=None, port: int = 0) -> Container:
